...
echo "# Startupscript is now running. Please be patient!"
echo "# -------------------------------------------------------"
...
if [ "$avahi" != "" ]; then echo "# AVAHI: "$avahi; fi
if [ "$multihost" != "" ]; then echo "# IOB_MULTIHOST: "$multihost; fi
if [ "$objectsdbhost" != "" ]; then echo "# IOB_OBJECTSDB_HOST: "$objectsdbhost; fi
if [ "$objectsdbport" != "" ]; then echo "# IOB_OBJECTSDB_PORT: "$objectsdbport; fi
if [ "$objectsdbtype" != "" ]; then echo "# IOB_OBJECTSDB_TYPE: "$objectsdbtype; fi
if [ "$packages" != "" ]; then echo "# PACKAGES: "$packages; fi
if [ "$setgid" != "" ]; then echo "# SETGID: "$setgid; fi
if [ "$setuid" != "" ]; then echo "# SETUID: "$setuid; fi
if [ "$statesdbhost" != "" ]; then echo "# IOB_STATESDB_HOST: "$statesdbhost; fi
if [ "$statesdbport" != "" ]; then echo "# IOB_STATESDB_PORT: "$statesdbport; fi
if [ "$statesdbtype" != "" ]; then echo "# IOB_STATESDB_TYPE: "$statesdbtype; fi
if [ "$usbdevices" != "" ]; then echo "# USBDEVICES: "$usbdevices; fi
if [ "$zwave" != "" ]; then echo "# ZWAVE: "$zwave; fi
echo "# -------------------------------------------------------"
...
then
  ...
  then
    ...
    echo "# For more information see readme.md on Github (https://github.com/dontobi/iobroker)."
    exit 1
  else
...
echo "# ioBroker successfully started... End of Startup Script"
echo "# -------------------------------------------------------"
